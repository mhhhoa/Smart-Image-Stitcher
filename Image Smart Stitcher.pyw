...
class ImageStitcherApp(ctk.CTk):
    def __init__(self):
        ...
        self.bg_color_menu = ctk.CTkOptionMenu(self.bg_row, values=list(self.color_palette.keys()) +["Transparent"], command=self.trigger_preview)
        ...
        ctk.CTkLabel(self.group_export, text="Формат:").pack(side="left", padx=10)
        self.format_menu = ctk.CTkOptionMenu(self.group_export, values=["JPG", "PNG"], width=100)
        self.format_menu.set("JPG")
        self.format_menu.pack(side="right", padx=10, pady=10)
    ...
    def create_group(self, title):
        ...
        label.pack(pady=(8, 5), padx=10, anchor="w")
        return frame
